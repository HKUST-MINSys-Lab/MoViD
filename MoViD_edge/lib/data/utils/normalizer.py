...
class Normalizer:
    ...
    def bbox_normalization(self, kp_2d, bbox, res, patch_width, patch_height):
        ...
        for idx in range(len(out_kp_2d)):
            out_kp_2d[idx] = transform_keypoints(
                kp_2d[idx], bbox[idx][:3], patch_width, patch_height
            )[0]
...
def normalize_keypoints_to_image(x, res):
    ...
    x = 2 * x / scale.reshape(*[1 for _ in range(len(x.shape[1:]))]) - mean.reshape(
        *[1 for _ in range(len(x.shape[1:-1]))], -1
    )
    return x
...
def compute_bbox_from_keypoints(X, do_augment=False, mask=None):
    ...
    def do_augmentation(scale_factor=0.2, trans_factor=0.05):
        scale_factor_value = np.random.uniform(1.0 - scale_factor, 1.2 + scale_factor)
        trans_x = np.random.uniform(-trans_factor, trans_factor)
        trans_y = np.random.uniform(-trans_factor, trans_factor)

        return scale_factor_value, trans_x, trans_y

    if do_augment:
        scale_factor, trans_x, trans_y = do_augmentation()
    else:
        scale_factor, trans_x, trans_y = 1.2, 0.0, 0.0
