...
class FreeMan:
    ...
    def __init__(self, base_dir, fps=25, split=''):
        ...
        # Init paths
        sub_dir = os.path.join(base_dir, f'videos_{int(fps)}fps')
        self.video_dir = os.path.join(base_dir, f'videos_{int(fps)}fps/')
        self.camera_dir = os.path.join(base_dir, 'cameras/')
        self.motion_dir = os.path.join(base_dir, 'motions/')
        self.keypoints2d_dir = os.path.join(base_dir, 'keypoints2d/')      # kpts2d_reproj_original
        self.keypoints3d_dir = os.path.join(base_dir, 'keypoints3d/')
        #self.bbox2d_dir = os.path.join(sub_dir, 'bbox2d/')
        self.filter_file = os.path.join(base_dir, 'ignore_list.txt')
        if len(split) == 0:
            self.session_list = self._sessionfile_to_list(os.path.join(base_dir, 'session_list.txt'))
        elif split == 'train':
            self.session_list = self._sessionfile_to_list(os.path.join(base_dir, 'train.txt'))
        elif split == 'validation':
            self.session_list = self._sessionfile_to_list(os.path.join(base_dir, 'validation.txt'))
        elif split == 'test':
            self.session_list = os.listdir(sub_dir)
            #self.session_list = self._sessionfile_to_list(os.path.join(base_dir, 'test.txt'))
    ...
    def load_bbox(self, keypoints2d, mask=None):
        return compute_bbox_from_keypoints(keypoints2d[...,:2], do_augment=True, mask=mask)
    ...
    @classmethod
    def load_motion(cls, motion_dir, seq_name,view_id):
        """Load a motion sequence represented using SMPL format."""
        # file_path = os.path.join(motion_dir, f'{seq_name}.pkl')
        file_path = os.path.join(motion_dir, f'{seq_name}_view{view_id}.npy')
        ...
        betas = data['smpl_betas']  # (1, 10)
        return smpl_poses, smpl_scaling, smpl_trans, betas
