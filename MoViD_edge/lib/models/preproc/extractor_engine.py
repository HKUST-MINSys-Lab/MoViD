...
from .backbone.hmr2_engine import HMR2Engine
...
class FeatureExtractor(object):
    def __init__(self, device, flip_eval=False, max_batch_size=64):
        ...
        self.engine = HMR2Engine("hmr2.engine")
    ...
    def predict_init(self, norm_img, tracking_results, _id, flip_eval=False):
        ...
        flat_out = self.engine.infer(norm_img)[0]  # shape: (229,)
        flat_out = np.array(flat_out, dtype=np.float32)

        # reshape back
        pred_global_orient = torch.tensor(flat_out[0:9]).reshape(1, 1, 3, 3)
        pred_body_pose = torch.tensor(flat_out[9:216]).reshape(1, 23, 3, 3)
        pred_betas = torch.tensor(flat_out[216:226]).reshape(1, 10)
        pred_cam = torch.tensor(flat_out[226:229]).reshape(1, 3)
