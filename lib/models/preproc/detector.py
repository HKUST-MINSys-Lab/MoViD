...
# Fix for PyTorch 2.6 compatibility: patch ultralytics torch.load before importing YOLO
try:
    import ultralytics.nn.tasks as ultralytics_tasks
    original_torch_safe_load = ultralytics_tasks.torch_safe_load
    
    def patched_torch_safe_load(weight):
        # Original function returns (ckpt, weight), so we need to match that signature
