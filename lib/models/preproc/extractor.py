...
class FeatureExtractor(object):
    ...
    def run(self, video, tracking_results, patch_h=256, patch_w=256):
        ...
        if not isinstance(video, list) and osp.isfile(video):
        ...
        # Track valid frame indices for each subject
        valid_frame_indices = {_id: [] for _id in tracking_results.keys()}

        # Initialize lists for features (and flipped data if needed)
        for _id in tracking_results.keys():
            tracking_results[_id]['features'] = []
            if self.flip_eval:
        ...
        while True:
            ...
            for _id, val in tracking_results.items():
                ...
                # Skip frames with invalid bbox (NaN values from missing keypoints)
                if np.isnan(cx) or np.isnan(cy) or np.isnan(scale) or scale <= 0:
                    continue

                # Track this frame as valid
                valid_frame_indices[_id].append(frame_id2)
                ...
                is_first_valid_frame = len(valid_frame_indices[_id]) == 1
        ...
        # Filter tracking_results to only include valid frames
        for _id, valid_indices in valid_frame_indices.items():
            if len(valid_indices) > 0:
                valid_indices = np.array(valid_indices)
                tracking_results[_id]['frame_id'] = tracking_results[_id]['frame_id'][valid_indices]
                tracking_results[_id]['bbox'] = tracking_results[_id]['bbox'][valid_indices]
                tracking_results[_id]['keypoints'] = tracking_results[_id]['keypoints'][valid_indices]
    ...
    def process(self, tracking_results):
        ...
        for _id, results in tracking_results.items():
            # Skip subjects with no valid frames
