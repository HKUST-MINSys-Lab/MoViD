...
# merged it into libtorch_cuda.so. Preload the library with ctypes.
try:
    import ctypes
    ...
    if torch_lib_path:
        ...
        # Preload libtorch_cuda.so so it's available when PyTorch3D looks for libtorch_cuda_cu.so
        libtorch_cuda_path = osp.join(torch_lib_path, 'libtorch_cuda.so')
        if osp.exists(libtorch_cuda_path):
            try:
                # Load with RTLD_GLOBAL so symbols are available globally
                ctypes.CDLL(libtorch_cuda_path, mode=ctypes.RTLD_GLOBAL)
                # Also try to create a "virtual" libtorch_cuda_cu.so by loading libtorch_cuda.so
                # and making it available under the expected name
                # This works because the symbols are the same
                ctypes.CDLL(libtorch_cuda_path)
            except Exception as e:
                # If library is already loaded or other error, continue
                pass
except Exception:
    # If ctypes fix fails, continue anyway - might work if LD_LIBRARY_PATH is set
    pass
