...
def main(cfg, args):
    ...
    with torch.no_grad():
        for i in range(len(eval_loader)):
            ...
            summary_string = f'{batch["vid"]} | PA-MPJPE: {pa_mpjpe.mean():.1f}   MPJPE: {mpjpe.mean():.1f}   PVE: {pve.mean():.1f} View Cosine: {view_metrics["view_feat_cosine_similarity"]:.4f}  View MSE: {view_metrics["view_feat_mse"]:.4f}'
