    ...
    with torch.no_grad():
        for i in range(len(eval_loader)):
            ...
            if cfg.FLIP_EVAL:
                ...
                f_x, f_inits, f_features, f_kwargs, f_gt = prepare_batch(flipped_batch, cfg.DEVICE, cfg.TRAIN.STAGE=='stage2' or cfg.TRAIN.STAGE=='stage3')
            
                # Forward pass with flipped input
                flipped_pred = network(f_x, f_gt, f_inits, f_features, **f_kwargs)
                time_dict['inference_flipped'] = time() - _t; _t = time()
                
            # Forward pass with normal input
            pred = network(x, gt, inits, features, **kwargs)
            ...
            target_output = smplx(
                ...
                betas=gt['betas'][0])
            view_metrics = network.evaluate_view_feature(target_output.joints[:, :17])
            ...
            accumulator['view_feat_cosine_similarity'].append(np.array([view_metrics['view_feat_cosine_similarity']]))
            accumulator['view_feat_mse'].append(np.array([view_metrics['view_feat_mse']]))
