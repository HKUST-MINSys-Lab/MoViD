...
def get_optimizer(cfg, model, optim_type, momentum, stage):
    if stage == 'stage2':
        #param_list = [{'params': model.lightweight_mlp.parameters()}]#{'params': model.view_encoder.parameters()},{'params': model.gated_fusion.parameters()},{'params': model.clip_gated_fusion.parameters()}, {'params': model.clip_proj.parameters()}]#, {'params': model.integrator.parameters()}]
        #param_list = [{'params': model.pose_proj.parameters()}, {'params': model.clip_gated_fusion.parameters()}, {'params': model.clip_proj.parameters()}, {'params': model.integrator.parameters()}, {'params': model.gated_fusion.parameters()}, {'params': model.dynamic_projection.parameters()}, {'params': model.motion_proj.parameters()}, {'params': model.view_encoder.parameters()}]
        param_list = [{'params': model.motion_encoder.parameters(), 'lr': cfg.TRAIN.LR_FINETUNE}]
        for name, param in model.named_parameters():
            #if 'gating_module' not in name:
            #if 'pose_proj' not in name and 'clip_gated_fusion' not in name and 'clip_proj' not in name and 'integrator' not in name and 'gated_fusion' not in name and 'dynamic_projection' not in name and 'view_encoder' not in name and 'motion_proj' not in name:
            if 'motion_encoder' not in name:
                param_list.append({'params': param, 'lr': cfg.TRAIN.LR})
...
def prepare_groundtruth(batch, device):
    ...
    gt_keys += ['pose_root', 'vel_root', 'weak_kp2d', 'verts',  # Training
                ...
                'has_smpl', 'has_traj', 'has_full_screen', 'has_verts','joints_3d','bone_vectors']
