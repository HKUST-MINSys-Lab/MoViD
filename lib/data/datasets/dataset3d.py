...
class Dataset3D(BaseDataset):
    ...
    def get_labels(self, index, target):
        ...
        elif self.__name__ == 'HuMMan' or self.__name__ == 'Freeman': # HuMMan, Freeman and TotalCapture have SMPL labels
            gt_kp3d = torch.zeros((self.n_frames, self.n_joints + 14, 3))
            gt_kp2d = torch.zeros((self.n_frames - 1, self.n_joints + 14, 2))

        elif self.__name__ == 'TotalCapture':
            gt_kp3d = torch.zeros((self.n_frames, self.n_joints + 14, 3))
            #gt_kp3d[:, self.n_joints:] = convert_kps(self.labels['joints_3d'][start_index:end_index+1], 'spin', 'common')
            gt_kp2d = torch.zeros((self.n_frames - 1, self.n_joints + 14, 2))
        ...
        if self.__name__ == 'TotalCapture':
            bone_list = []
            bone_vectors = self.labels['bone_vectors'][start_index:end_index+1]
            for i in range(len(bone_vectors)):
                bone_array = np.array([v for v in bone_vectors[i].values()])
                bone_list.append(torch.from_numpy(bone_array).float()) 
            bone_list = torch.stack(bone_list, dim=0)
            target['bone_vectors'] = bone_list
            #print(target['bone_vectors'].shape)
        else:
            target['bone_vectors'] = torch.zeros((self.n_frames, 11, 3)).float()
            #print(target['bone_vectors'].shape)
        
        if self.__name__ != 'ThreeDPW' and self.__name__ != 'HuMMan' and  self.__name__ != 'Freeman' and self.__name__ != 'TotalCapture': # 3DPW does not contain world-coordinate motion
    ...
    def get_camera_info(self, index, target):
        ...
        if self.__name__ == 'Human36M' or self.__name__ == 'TotalCapture':
    ...
    def get_single_sequence(self, index):
        ...
        target = {'has_full_screen': torch.tensor(True),
                  ...
                  'has_imu': torch.tensor(self.has_imu),
