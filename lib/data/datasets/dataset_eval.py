...
class EvalDataset(BaseDataset):
    def __init__(self, cfg, data, split, backbone):
        ...
        #self.labels = torch.load('dataset/parsed_data/totalcapture_processed_val.pth')
    ...
    def prepare_labels(self, index, target):
        ...
        if self.labels['pose'][index] is None:
            return None
    ...
    def prepare_inputs(self, index, target):
        ...
        target['mask'] = mask[1:]
        if 'joints_3d' in self.labels:
            target['joints_3d'] = self.labels['joints_3d'][index]
