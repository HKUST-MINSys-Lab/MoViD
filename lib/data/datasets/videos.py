...
class Human36M(Dataset3D):
    def __init__(self, cfg, dset='train'):
        ...
        #parsed_data_path = os.path.join(_C.PATHS.PARSED_DATA, f'human36m_view012_{dset}_backbone_stacked.pth')
...
class FreemanDataset(Dataset3D):
    def __init__(self, cfg, dset='train'):
        parsed_data_path = os.path.join(_C.PATHS.PARSED_DATA, f'freeman_{dset}_backbone.pth')
        parsed_data_path = parsed_data_path.replace('backbone', cfg.MODEL.BACKBONE.lower())
        super(FreemanDataset, self).__init__(cfg, parsed_data_path, dset=='train')
        ...
        #self.mask[-14:] = 1
        self.smpl_gender = {
            -1: SMPL(_C.BMODEL.FLDR, gender='male', num_betas=10),
            0: SMPL(_C.BMODEL.FLDR, gender='neutral', num_betas=10),
...
class HuMManDataset(Dataset3D):
    def __init__(self, cfg, dset='train'):
        parsed_data_path = os.path.join(_C.PATHS.PARSED_DATA, f'humman_{dset}1469_backbone.pth')
        parsed_data_path = parsed_data_path.replace('backbone', cfg.MODEL.BACKBONE.lower())
        super(HuMManDataset, self).__init__(cfg, parsed_data_path, dset=='train')
...
class TotalCaptureDataset(Dataset3D):
    def __init__(self, cfg, dset='train'):
        parsed_data_path = os.path.join(_C.PATHS.PARSED_DATA, f'totalcapture_{dset}_backbone.pth')
        parsed_data_path = parsed_data_path.replace('backbone', cfg.MODEL.BACKBONE.lower())
        super(TotalCaptureDataset, self).__init__(cfg, parsed_data_path, dset=='train')
