...
class VideoAugmentor():
    ...
    def get_jitter(self, ):
        ...
        jittering_noise = torch.normal(
            ...
            std=self.aug_dict['jittering'][:num_joints].reshape(1, num_joints, 1).expand(self.l, -1, 3)
    ...
    def get_lfhp(self, ):
        ...
        def get_peak_noise_mask():
            peak_noise_mask = torch.rand(self.l, num_joints).float() * self.aug_dict['pmask'][:,:num_joints].squeeze(0)
        ...
        peak_noise = peak_noise * torch.randn(3) * self.aug_dict['peak'][ :num_joints].reshape(1, -1, 1) * _C.KEYPOINTS.S_PEAK
        return peak_noise

    def get_bias(self, ):
        """Bias noise modeling."""
        bias_noise = torch.normal(
            mean=torch.zeros((num_joints, 3)), std=self.aug_dict['bias'][:num_joints].reshape(num_joints, 1)
